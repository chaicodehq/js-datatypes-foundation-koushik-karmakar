...
export function findPassenger(passengers, name) {
  ...
  return passengers.find((f) => f.name.toLowerCase() === name.toLowerCase());
}

export function getPassengerIndex(passengers, name) {
  if (typeof name !== "string" || !Array.isArray(passengers)) return -1;
  return passengers.findIndex((f) => f.name.toLowerCase() === name.toLowerCase());
}
...
export function getWaitlistedPassengers(passengers) {
  ...
  return passengers.filter((f) => f.status === "waitlisted");
}
